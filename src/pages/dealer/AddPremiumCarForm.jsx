...
export default function AddPremiumCarForm() {
  ...
  const handleSubmit = async (event) => {
    ...
    const data = {
      ...
      variant: (formData.cVariant || "").trim(),
      ...
      // numbers kept as provided (backend accepts strings too)
    ...
    try {
      ...
      if (res?.data?.status === "success") {
        ...
        setTimeout(() => {
          navigate(`/dealer/${id}/uploadimage/${res?.message}`);
        }, 800);
      } else {
  ...
  return (
    <>
      <ToastContainer />
      <div className="md:flex justify-center mb-5  md:m-0">
        <div>
          <form onSubmit={handleSubmit} className="w-full md:w-[45rem]">
            <div className="flex justify-center">
              <p className="text-3xl font-semibold mt-12"> Add Premium Car </p>
            </div>
            {/* first part */}
            ...
            <button
              type="submit"
              className="p-3 mt-3 mb-3 bg-indigo-400 rounded-md w-28 text-white"
              value="Add  Car"
            >
              {" "}
              Next
